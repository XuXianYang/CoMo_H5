var emoji = (function(window, $) {
	...
	__.template = function(Id) {
		$.each(__.emojiData(), function(i, v) {
			$('<div class="swiper-slide"><div id="face_' + (i + 1) + '" class="content-tab-face selfclear clearfix" ></div>').appendTo('.swiper-wrapper');
		...
		//添加键与值
		__.addMap();
		$(".face-main-box").swiper({
			pagination: '.swiper-pagination'
		});
		$(".dx-emoji").css({"visibility":"visible"}).hide();
	};
	//点击插入内容
	__.addText = function() {
	...
	__.isShowEmoji = function() {
		if (!__.emojiBox.is(':visible')) {
			...
			__.emojiBox.hide();
      window.scrollTo(0,$(".J_page").height());
		}
	};
	//主入口
