...
(function( $ ) {
    ...
    $.fn.jsonEditor = function(json, options) {
        ...
        var onValueClick = options.onValueClick || K;

        return this.each(function() {
            JSONEditor($(this), json, onchange, onpropertyclick, options.propertyElement, options.valueElement, onValueClick);
        });
        
    };
    
    function JSONEditor(target, json, onchange, onpropertyclick, propertyElement, valueElement, onValueClick) {
        var opt = {
            target: target,
            onchange: onchange,
    ...
    function valueClicked(opt){
        return function(){
            var path = $(this).prev().parent().data('path');          
            var key = $(this).prev().attr('title');

            //var safePath = path ? path.split('.').concat([key]).join('"][\"') : key;
            var safePath = path ? path.split('.').concat([key]).join('","') : key;
            safePath = '["' + safePath + '"]';
            opt.onValueClick(safePath, $(this).val(), $(this));
        }
    }
