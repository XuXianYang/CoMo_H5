...
/**
 * Default {@link org.glassfish.jersey.server.internal.inject.ConfiguredValidator} implementation - delegates calls to the underlying {@link javax.validation.Validator}.
 *
 * @author Michal Gajdos
 * @author Jakub Podlesak (jakub.podlesak at oracle.com)
 */
public class DefaultConfiguredValidator implements ConfiguredValidator, ValidationInterceptor {
     ...
     * @param configuration              configuration to obtain {@link javax.validation.executable.ExecutableType executable types} configured in descriptor
     *                                   from.
     ...
     */
    public DefaultConfiguredValidator(final Validator delegate, final Configuration configuration,
    ...
    private List<ValidationInterceptor> createInterceptorList(Iterable<ValidationInterceptor> interceptors) {
        List<ValidationInterceptor> result = new LinkedList<ValidationInterceptor>();
    ...
    public void onValidate(final ValidationInterceptorContext ctx) {
        ...
        if (resourceMethod != null
            ...
            Annotation[][] parameterAnnotations = handlingMethod.getParameterAnnotations();
            if (null != parameterAnnotations) {
                for (int i = 0; i< parameterAnnotations.length; i++) {
                    Annotation[] annotations = parameterAnnotations[i];
                    if (null != annotations) {
                        for (Annotation annotation : annotations) {
                            if (annotation.annotationType().equals(BeanParam.class)) {
        ...
        if (!constraintViolations.isEmpty()) {
            throw new ConstraintViolationException(resourceMethod, constraintViolations);
        }
    }
