...
public class SwaggerJersey2Jaxrs extends AbstractSwaggerExtension {
    ...
    @Override
    public List<Parameter> extractParameters(Swagger swagger, final List<Annotation> annotations, final Type type, final Set<Type> typesToSkip, final Iterator<SwaggerExtension> chain) {
        ...
        for (final Annotation annotation : annotations) {
            if (annotation instanceof BeanParam) {
                ...
                for (final BeanPropertyDefinition propDef : properties) {
                    ...
                    if (null == paramType) {
                        // 假如一个bean里定义了一个getXXX方法，本身没有对应的field，也没有setter方法
                        // 那么paramType可能为null，会导致解析时抛Exception
                        continue;
                    }
                    ...
                    List<Parameter> extracted =
                            extensions.next().extractParameters(swagger, paramAnnotations, paramType, typesToSkip, extensions);
        ...
        if (parameters.isEmpty()) {
            parameters = super.extractParameters(swagger, annotations, type, typesToSkip, chain);
        }
