package com.dianxian.core.swagger.jaxrs.listing;

import com.dianxian.core.swagger.Reader;
import com.dianxian.core.utils.lang.StringUtils;
import com.dianxian.resource.HelloWorldResource;
...
//            classes.add(HelloWorldResource.class);
        ...
        synchronized (ApiListingResource.class) {
            if (SwaggerContextService.isScannerIdInitParamDefined(sc)) {
                if (!initializedScanner.containsKey(sc.getServletName() + "_" + SwaggerContextService.getScannerIdFromInitParam(sc))) {
                    ...
                    swagger.setBasePath(getBasePath(uriInfo));
                }
            } else {
    ...
    protected String getBasePath(UriInfo uriInfo) {
        String[] splittedPath = uriInfo.getAbsolutePath().getPath().split("/");
        List<String> tempPathList = new ArrayList<String>();
        for (int i = 0; i < splittedPath.length - 1; i++) {
            tempPathList.add(splittedPath[i]);
        }
        return StringUtils.join(tempPathList, "/");
    }
